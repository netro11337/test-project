 ...
 * Режим Б — запасной.
 *
 * Собирает файл остатков с нуля, без шаблона из личного кабинета.
 * Колонки берутся из CONFIG.OUT_*_HEADER — сверьте их с шаблоном Ozon
 * перед первой загрузкой.
 *
 * Режим А (Fill.gs) надёжнее: там формат гарантированно совпадает
 * с тем, что ждёт личный кабинет.
 */
function buildStockFile() {
  ...
  try {
    ...
    for (var i = 0; i < source.rows.length; i++) {
      rows.push([CONFIG.WAREHOUSE_NAME, source.rows[i].raw, source.rows[i].qty]);
    }
