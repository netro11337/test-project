...
class Settings:
    ...
    # Программа сама открывает недостающие браузеры. Батник остаётся запасным
    # вариантом.
    auto_launch_browsers: bool = True

    # Сколько окон поднимать. Меньше, чем потоков, — оставшиеся вкладки идут
    # в тех же окнах кругами: корзина за корзиной, с очисткой между ними.
    # Один браузер на один аккаунт — единственный способ не смешать корзины,
    # потому что корзина магазина привязана к аккаунту, а не к окну.
    browser_count: int = 1
