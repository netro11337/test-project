...
rem Файл сохранён в кодировке cp866 и НЕ переключает кодировку через chcp:
rem смена кодировки посреди batch-файла сбивает разбор команд.

if not exist "main.py" (
    echo.
    echo Рядом с этим файлом нет main.py.
    echo Положите этот файл в папку, где лежат main.py и папка cart_bot.
    echo.
    pause
    exit /b 1
)
